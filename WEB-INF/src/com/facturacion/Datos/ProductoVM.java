...
public class ProductoVM extends TemplateViewModelLocal {
	...
	public void modalProductos(@BindingParam("productoid") long productoid) {
		...
		if (productoid != -1) {
			...
			this.editar = true;
			this.buscarMarca = this.productoSelected.getMarcaFK().getMarca();
		...
		modal = (Window) Executions.createComponents("/sistema/zul/Datos/productosModal.zul", this.mainComponent, null);
	...
	private List<Object[]> lMarcaBuscar;
	private List<Object[]> lMarcaBuscarOri;
	private Marca marcaSelected;

	@Command
	@NotifyChange("lMarcaBuscar")
	public void filtrarMarcaBuscar() {
		this.lMarcaBuscar = this.filtrarListaObject(buscarMarca, lMarcaBuscarOri);
	}

	@Command
	@NotifyChange("lMarcaBuscar")
	public void generarListaBuscarMarca() {
		this.lMarcaBuscar = this.reg.sqlNativo("SELECT m.marcaid, m.marca FROM marcas m ORDER BY m.marcaid asc");
		this.lMarcaBuscarOri = this.lMarcaBuscar;
	}

	@Command
	@NotifyChange("buscarMarca")
	public void onSelectMarca(@BindingParam("id") Long id) {
		this.marcaSelected = this.reg.getObjectById(Marca.class.getName(), id);
		this.productoSelected.setMarcaFK(marcaSelected);
		this.buscarMarca = this.marcaSelected.getMarca();
	...
	public List<Object[]> getlMarcaBuscar() {
		return lMarcaBuscar;
	}

	public void setlMarcaBuscar(List<Object[]> lMarcaBuscar) {
		this.lMarcaBuscar = lMarcaBuscar;
	}
